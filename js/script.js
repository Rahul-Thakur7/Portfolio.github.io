...
let commandList = ['whoami','from','about','skills', 'resume','projects','socials','clear', 'echo',];
...
function checkCommand(inputCommand){
    ...
    if(command){
        ...
        else if(command === 'about'){ commandAbout(command);}
        else if(command === 'echo') {commandEcho(inputCommand);}

        else if(command === 's3cret'){ commandsecret();}
        else if(command === 'resume'){ commandResume();}
        else if(commandList.includes(command)){ executeCommand(command);}
        else{
            terminalBody.innerHTML +=  '<br>' + inputCommand + ' is not recognized as a command,  you know some developers keep  cradensial in source code <br>';
        }
    }
    else {
...
function commandAbout(){
    terminalBody.innerHTML += `<br>${data.about[0].value}<br>`;
...
function commandFrom(){
    terminalBody.innerHTML += `<br>${data.localhost[0].value}<br>`;
